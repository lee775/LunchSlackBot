...
class KakaoSlackBot {
  ...
  async syncProfileImage() {
    logger.info('Starting multiple image sync...');
    
    try {
      // 1. 카카오톡 플러스친구에서 모든 이미지 가져오기
      logger.info('Fetching all images from KakaoTalk Plus Friend...');
      const allImages = await this.kakaoScraper.getAllImages(config.kakao.plusFriendUrl);
      
      if (!allImages || allImages.length === 0) {
        throw new Error('Failed to get any images');
      }

      logger.info(`Fetched ${allImages.length} images:`);
      allImages.forEach((img, i) => {
        logger.info(`${i + 1}. Type: ${img.type}, Method: ${img.method}, Size: ${img.size || 'unknown'}`);
      });

      // 2. Slack에 모든 이미지 업로드
      logger.info('Uploading multiple images to Slack...');
      const uploadResult = await this.slackClient.uploadMultipleImages(
      ...
      if (uploadResult.success) {
        logger.info(`Multiple image sync completed successfully. Total images: ${uploadResult.totalImages}`);
        uploadResult.results.forEach((result, i) => {
          logger.info(`Image ${i + 1} (${result.type}): File ID ${result.fileId}`);
        });
        
        return {
          success: true,
          timestamp: new Date().toISOString(),
          totalImages: uploadResult.totalImages,
          results: uploadResult.results
        };
      } else {
        throw new Error('Failed to upload images to Slack');
      }

    } catch (error) {
      logger.error('Multiple image sync failed:', error);
